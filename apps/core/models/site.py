"""Core models — Site, GA4EventDefinition, KPIGoal, WeeklySnapshot."""
...
class Site(models.Model):
    ...
    project = models.ForeignKey(
        ...
        verbose_name="Projeto", null=True, blank=True,
    )
    ...
    # GA4
    ga4_measurement_id = models.CharField(max_length=30, blank=True, verbose_name="GA4 Measurement ID", help_text="Ex: G-BCGGTGQJR9")
    ga4_api_secret = models.CharField(max_length=100, blank=True, verbose_name="GA4 API Secret", help_text="Measurement Protocol API secret")
    ga4_property_id = models.CharField(max_length=30, blank=True, verbose_name="GA4 Property ID", help_text="Para GA4 Data API (reporting)")
    ...
    # Google OAuth
    google_refresh_token = models.TextField(blank=True, verbose_name="Google Refresh Token", help_text="OAuth refresh token for GA4 + GSC APIs")
    google_access_token = models.TextField(blank=True, verbose_name="Google Access Token", help_text="Cached OAuth access token")

    # Legacy Service Account Keys
    gsc_service_account_key = models.TextField(blank=True, verbose_name="GSC Service Account JSON", help_text="Conteúdo completo do JSON da Service Account do Google Search Console")
    ga4_service_account_key = models.TextField(blank=True, verbose_name="GA4 Service Account JSON", help_text="Conteúdo completo do JSON da Service Account do GA4 Data API")
...
class GA4EventDefinition(models.Model):
    ...
    site = models.ForeignKey(Site, on_delete=models.CASCADE, related_name="event_definitions")
    event_name = models.CharField(max_length=100, verbose_name="Nome do Evento", help_text="Ex: sign_up, generate_lead, purchase")
    description = models.TextField(blank=True, verbose_name="Descrição")
    trigger_page = models.CharField(max_length=500, blank=True, verbose_name="Página de Disparo", help_text="Template ou URL onde o evento deve ser disparado")
    priority = models.CharField(max_length=10, choices=PRIORITY_CHOICES, default="medium")
    js_snippet = models.TextField(blank=True, verbose_name="Código JavaScript", help_text="Snippet gtag() para copiar no template")
    server_side = models.BooleanField(default=False, verbose_name="Server-Side (Measurement Protocol)")
    parameters = models.JSONField(default=dict, blank=True, verbose_name="Parâmetros", help_text="Parâmetros padrão do evento em JSON")
...
class KPIGoal(models.Model):
    ...
    PERIOD_CHOICES = [
        ("month_1", "Mês 1"), ("month_3", "Mês 3"),
        ("month_6", "Mês 6"), ("month_12", "Mês 12"),
    ]

    site = models.ForeignKey(Site, on_delete=models.CASCADE, related_name="kpi_goals")
    metric_name = models.CharField(max_length=200, verbose_name="Nome da Métrica", help_text="Ex: Sessões Orgânicas, Sign-ups, Receita Orgânica")
    source = models.CharField(max_length=100, verbose_name="Fonte", help_text="Ex: GA4, GSC, Admin")
    ...
    unit = models.CharField(max_length=20, blank=True, default="", help_text="Ex: R$, %, unidades")
    updated_at = models.DateTimeField(auto_now=True)
...
class WeeklySnapshot(models.Model):
    ...
    site = models.ForeignKey(Site, on_delete=models.CASCADE, related_name="weekly_snapshots")
    ...
    revenue = models.DecimalField(max_digits=12, decimal_places=2, default=0, verbose_name="Receita (R$)")
    ...
    gsc_ctr = models.DecimalField(max_digits=5, decimal_places=2, default=0, verbose_name="CTR (%)")
    gsc_position = models.DecimalField(max_digits=5, decimal_places=1, default=0, verbose_name="Posição Média")
